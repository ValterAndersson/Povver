...
| File | Purpose |
|------|---------|
| `agent.py` | ShellAgent class: ADK agent definition (gemini-2.5-flash, temp 0.3), before_model/before_tool callbacks for context injection, after_model_callback for per-turn usage tracking via direct `track_usage()` calls (ContextVar doesn't work across ADK's thread boundary) |
