...
- `startSSEStream()` — Begin agent streaming via `DirectStreamingService`. Injects a local `user_prompt` into `workspaceEvents` for instant display; the server persists the authoritative copy to `workspace_entries` via Admin SDK (client writes are blocked by Firestore security rules). Send button is disabled while `isAgentThinking` to prevent cancelling in-flight streams.
...
- Canvas metadata (`updatedAt`, `lastMessage`) — updated for recent-chats listing

**Firestore Listeners:**
- Workspace events — queries newest 200 entries (descending), reversed to chronological order for display. Replaces any locally-injected entries when the listener fires.
